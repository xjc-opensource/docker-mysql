...
if [ ! -f "$backupDir"/latest_full ]; then
        ...
        lastFull=`cat "$backupDir"/latest_full`
        /usr/bin/innobackupex --defaults-file=$file_cnf  --socket=$socket --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --rsync "$backupDir"/"$dateNow"_full > $backupLog 2>&1
else
        ...
        if [ $difference -lt $hoursBeforeFull ]; then
                ...
                /usr/bin/innobackupex --defaults-file=$file_cnf --socket=$socket  --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --rsync --incremental --incremental-basedir="$backupDir"/"$lastFullDir"_full "$backupDir"/"$dateNow"_incr > $backupLog 2>&1
        else
